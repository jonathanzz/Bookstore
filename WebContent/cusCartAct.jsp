...
<title>Customer Car Activity</title>
<!-- Bootstrap -->
...
<body>
	...
	<div class="container-fluid">
			...
			<div class="span9" id="content">
					...
					<div class="navbar">
						<div class="navbar-inner">
							<ul class="breadcrumb">
								...
								<li class="active">Customer's Cart Activity<span
									class="divider">/</span></li>
							</ul>
						</div>
					</div>
				...
				<%
					...
					for (MonitorTable m : hehe) {
						if (m.isAdded() == true || m.isRemoved() == true)
							activities.add(m);
					}
						...
						<div class="block">
							<div class="navbar navbar-inner block-header">
								<div class="muted pull-left"><%=username%>
									Shopping Cart Activity Details:
								</div>
										...
										<%
											int current_page = 1;
												...
												for (int i = current_page * 15 - 15; i < current_page * 15 - 15
													...
													title = DAOFactory.getIBooksDAOInstance()
															.findByBookkey(item.getBook()).getTitle();
										%>
										<tr>
											<td><%=i + 1%></td>
											<td><%=item.getProcessdate()%></td>
											<td><a href='detail.jsp?key=<%=item.getBook()%>'><%=title%></a></td>
											<%
												if (item.isAdded()) {
											%>
											<td>Add to cart</td>
		...
		<center>
			...
			%>
			<a
				href='<%=request.getContextPath()%>/cusCartAct.jsp?username=<%=username%>&&det=<%=current_page - 1%>'>Pre</a>
			<%
				}
			%>
			<%
			...
			%>
			<a
				href='<%=request.getContextPath()%>/cusCartAct.jsp?username=<%=username%>&&det=<%=i%>'><%=i%></a>
			...
			%>
			<a
				href='<%=request.getContextPath()%>/cusCartAct.jsp?username=<%=username%>&&det=<%=current_page + 1%>'>Next</a>
			<%
				}
			%>
		</center>
